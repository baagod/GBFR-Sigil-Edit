...
- If nothing changes in game, read `<Reloaded-II>\Mods\GBFR.SkillEdit\GBFR.SkillEdit.log`: the mod records which rows it changed and says so when it cannot find `Config.json` (it falls back to `%TEMP%` when that folder is not writable).
