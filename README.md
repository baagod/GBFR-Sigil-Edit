...
- **A battle already under way keeps the numbers it started with**: the game settles a trait's parameters into its own structures when a fight begins and then reads those for the rest of it, so an edit made mid-battle is used from the next battle on - the trait's description changes immediately all the same. Everything outside a battle, and every battle started after the edit, uses the new values with no restart.
