...
- **添加因子**：在搜索框中按名称或 hash 找到它，勾选它所在的行（或其中某一等级）即启用。该等级的游戏原值以灰字占位符显示，**你改哪个槽就写哪个槽**，没输入的槽不写进配置、游戏里保持它自己的数值。**输入数值不会自动启用**——勾选才是让它在游戏里生效的开关。配置里只保留"已勾选、或你输入过数值"的等级；既没勾选也没输入过的等级不写进配置（勾了又取消，或把输入过的数值全部清空，都会把它移除）。
