...
- **添加因子**：在搜索框中按名称或 hash 找到它，勾选它所在的行（或其中某一等级）即启用。条目带着该因子词条的原始值，只需改你想改的那个数值。**输入数值不会自动启用**——勾选才是让它在游戏里生效的开关。配置里保留的是"已勾选、或你输入过数值"的等级；既没勾选也没输入过的等级不写进配置（勾了又取消，或把输入过的数值全部清空，都会把它移除）。
