...
常见的改表 mod 会自带一份改好的 `.tbl` 覆盖游戏原文件，两个这样的 mod 改同一张表就会互相覆盖；本 mod 从游戏封包里读出表，只改你指定的行，再通过 `IDataManager` 交回去，磁盘上什么都没变。

不过粒度仍然是「一个路径一份文件」：`IDataManager` 对 `system/table/skill_status.tbl` 只保留一份生效内容，所以本 mod 与另一个**同样提供这张表**的 mod 不会按行合并 —— 谁最后注册，游戏解析到的就是谁的版本，而本 mod 每次改动都会重新注册一次。改别的表的 mod 不受影响。
